  ...
  return (
    <div className="page-container">
      <div className="container py-3">
        <button className="btn btn-link text-brown p-0 mb-3" onClick={() => navigate('/orders')}>
        ...
        {isNewOrder && (
          <div className="alert alert-success d-flex align-items-center mb-4" role="alert">
            ...
            <div>
              <strong>{t('order_success')}</strong>
              <p className="mb-0 small">{t('order_success_desc')}</p>
            </div>
          </div>
        )}
        ...
        <div className="card-pahala card mb-3">
          <div className="card-body">
            <h6 className="fw-bold mb-3"><i className="bi bi-box me-2"></i>{t('order_items')}</h6>
            {order.items.map((item, index) => (
              <div key={index} className="d-flex gap-3 mb-3 pb-3 border-bottom">
                <img src={item.product.image} alt={item.product.name} className="rounded" style={{ width: '60px', height: '60px', objectFit: 'cover' }} onError={(e) => { (e.target as HTMLImageElement).src = '/placeholder.svg'; }} />
                <div className="flex-grow-1">
                  <h6 className="mb-1 small fw-semibold">{item.product.name}</h6>
                  <div className="text-muted small">{item.quantity} {item.product.unit} × {formatPrice(item.product.price)}</div>
                </div>
                <div className="fw-semibold text-brown">{formatPrice(item.product.price * item.quantity)}</div>
              </div>
            ))}
        ...
        <div className="card-pahala card mb-3">
          <div className="card-body">
            <h6 className="fw-bold mb-3"><i className="bi bi-credit-card me-2"></i>{t('order_payment')}</h6>
            <div className="d-flex align-items-center gap-2">
              <i className="bi bi-cash-coin text-brown"></i>
              <span>{t('cart_cod')}</span>
            </div>
          </div>
        </div>
        ...
        {order.notes && (
          <div className="card-pahala card mb-3">
            <div className="card-body">
              <h6 className="fw-bold mb-3"><i className="bi bi-sticky me-2"></i>{t('order_notes')}</h6>
        ...
        <div className="card-pahala card">
          <div className="card-body text-center">
            <p className="mb-3 text-muted small">{t('order_help')}</p>
